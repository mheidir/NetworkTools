...
public class SnmpRequest implements CommandResponder, PDUFactory {
  ...
  private static boolean listeningPortUp = false;
  ...
  public synchronized void listen() throws IOException {
    if (address instanceof TcpAddress) {
      transport = new DefaultTcpTransportMapping((TcpAddress) address);
        ...
        try {
            ...
            System.out.println("Unable to establish listening port. Port is either in used or permission denied by operating system.");
        }
    }
    ...
    try {
        System.out.println("Trap receiver listening on "+address);
        this.wait();
  ...
  public synchronized void stop() {
      System.out.println("Trap receiver shutting down...");
      try {
            transport.close();
            System.out.println("--------------- Done ---------------\n");
        } catch (IOException ex) {
            //Logger.getLogger(TrapReceiver.class.getName()).log(Level.SEVERE, null, ex);
            System.out.println("Unable to close listening port on " + address);
        }
  }
  ...
  public PDU send() throws IOException {
    ...
    if ((operation == WALK) || (operation == SNAPSHOT_CREATION)) {
      ...
      if (responseEvent != null) {
        ...
        System.out.println(SnmpUtils.currentDate() + ": Response time: ("+
  ...
  protected static void printVersion() {
    System.out.println();
    System.out.println("SNMP4J Command Line Tool v1.9.1 Copyright "+(char)0xA9+
                       " 2004-2008, Frank Fock and Jochen Katz");
  ...
  protected static void printVariableBindings(PDU response) {
    for (int i=0; i<response.size(); i++) {
      ...
      System.out.println(SnmpUtils.currentDate() + ": Value (" + vb.toString() + ")");
    }
  }

  protected static void printReport(PDU response) {
    if (response.size() < 1) {
      System.out.println(SnmpUtils.currentDate() + ": REPORT PDU does not contain a variable binding.");
      return;
    }
    ...
    if (SnmpConstants.usmStatsUnsupportedSecLevels.equals(oid)) {
      System.out.println(SnmpUtils.currentDate() + ": REPORT: Unsupported Security Level.");
    }
    else if (SnmpConstants.usmStatsNotInTimeWindows.equals(oid)) {
      System.out.println(SnmpUtils.currentDate() + ": REPORT: Message not within time window.");
    }
    else if (SnmpConstants.usmStatsUnknownUserNames.equals(oid)) {
      System.out.println(SnmpUtils.currentDate() + ": REPORT: Unknown user name.");
    }
    else if (SnmpConstants.usmStatsUnknownEngineIDs.equals(oid)) {
      System.out.println(SnmpUtils.currentDate() + ": REPORT: Unknown engine id.");
    }
    else if (SnmpConstants.usmStatsWrongDigests.equals(oid)) {
      System.out.println(SnmpUtils.currentDate() + ": REPORT: Wrong digest.");
    }
    else if (SnmpConstants.usmStatsDecryptionErrors.equals(oid)) {
      System.out.println(SnmpUtils.currentDate() + ": REPORT: Decryption error.");
    }
    else if (SnmpConstants.snmpUnknownSecurityModels.equals(oid)) {
      System.out.println(SnmpUtils.currentDate() + ": REPORT: Unknown security model.");
    }
    else if (SnmpConstants.snmpInvalidMsgs.equals(oid)) {
      System.out.println(SnmpUtils.currentDate() + ": REPORT: Invalid message.");
    }
    else if (SnmpConstants.snmpUnknownPDUHandlers.equals(oid)) {
      System.out.println(SnmpUtils.currentDate() + ": REPORT: Unknown PDU handler.");
    }
    else if (SnmpConstants.snmpUnavailableContexts.equals(oid)) {
      System.out.println(SnmpUtils.currentDate() + ": REPORT: Unavailable context.");
    }
    else if (SnmpConstants.snmpUnknownContexts.equals(oid)) {
      System.out.println(SnmpUtils.currentDate() + ": REPORT: Unknown context.");
    }
    else {
      System.out.println(SnmpUtils.currentDate() + ": REPORT contains unknown OID ("
                         + oid.toString() + ").");
    }
    System.out.println(SnmpUtils.currentDate() + ": Current counter value is " +
    ...
    if (command != null) {
      ...
      if ((command.getType() != PDU.TRAP) &&
        ...
        try {
          ...
          System.out.println(SnmpUtils.currentDate() + ": Error while sending response: "+ex.getMessage() + "\n");
          ...
          if (snmpRequest.operation == LISTEN) {
            ...
            if ((snmpRequest.getPduType() == PDU.TRAP) ||
              ...
              System.out.println(">> " + PDU.getTypeString(snmpRequest.getPduType()) +
                                 " sent successfully to " + snmpRequest.getAddress().toString());
            }
            else if (response == null) {
              if (snmpRequest.operation != WALK) {
                System.out.println(SnmpUtils.currentDate() + ": Request timed out.");
              }
            }
              ...
              printReport(response);
                ...
                System.out.println(SnmpUtils.currentDate() + ": RequestID (" + response.getRequestID() +")");
                System.out.println(SnmpUtils.currentDate() + ": ErrorIndex (" + response.getErrorIndex() + ") ");
                System.out.println(SnmpUtils.currentDate() + ": ErrorStatus (" + response.getErrorStatusText() + ")");
                printVariableBindings(response);
            }
            else {
              System.out.println(SnmpUtils.currentDate() + ": Received unknown data");
              System.out.println(SnmpUtils.currentDate() + ": RequestID (" + response.getRequestID() +")");
              System.out.println(SnmpUtils.currentDate() + ": ErrorIndex (" + response.getErrorIndex() + ") ");
              System.out.println(SnmpUtils.currentDate() + ": ErrorStatus (" + response.getErrorStatusText() + "(" + response.getErrorStatus() + ")");
              printVariableBindings(response);
            }
          }
        }
        catch (IOException ex) {
          //System.err.println(SnmpUtils.currentDate() + ": Error while trying to send request: " + ex.getMessage());
          System.out.println(SnmpUtils.currentDate() + ": Error while trying to send request: " + ex.getMessage());
          ex.printStackTrace();
        }
      }
    }
    catch (IllegalArgumentException iaex) {
      //System.err.print(SnmpUtils.currentDate() + ": Error: "+iaex.getMessage());
      System.out.print(SnmpUtils.currentDate() + ": Error: "+iaex.getMessage() + "\n");
